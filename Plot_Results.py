...
#filename=raw_input()
filename='Out_Files/Dlc_11T_+00deg_V04_S1.out'
#while answer=='j':
for ii in range(2):

    if ii == 1:
        #filename='Out_Files/All_DLCs_idling_extreme_loads_DISS_CalcTwrAero_True/Dlc_62T_+80deg_S3.out'
        filename='Out_Files/All_DLCs_parked_extreme_loads_DISS_CalcTwrAero_False_Pitchrate_Parking_one_Pitch_Angle_Modified_2deg_s/Dlc_64T_+00deg_V10_S6.out'
        column2 = 8
        column3 = 9
    if ii == 0:
        filename='Out_Files/All_DLCs_parked_extreme_loads_DISS_CalcTwrAero_False_Pitchrate_Parking_one_Pitch_Angle_Modified_2deg_s/Dlc_64T_+00deg_V08_S6.out'
        column2 = 8
        column3 = 9
    ...
    lns1 = ax1.plot(np.transpose(data)[column1][0:],np.transpose(data)[column2][0:], 'b-', label = r'$\mathrm{'+names[column2] + '}$ / $\mathrm{' + units[column2] +'}$')
    # Make the y-axis label and tick labels match the line color.
    ax1.set_ylabel(r'$\mathrm{'+names[column2] + '}$ / $\mathrm{' + units[column2] +'}$', color='b')
    #lns1 = ax1.plot(np.transpose(data)[column1][0:],winddir, 'b-', label = r'$\mathrm{WindDir}$ / $\mathrm{m/s}$')
    #ax1.set_ylabel(r'$\mathrm{WindDir}$ / $\mathrm{deg}$', color='b')
    #if ii == 0:
    ...
    ax2 = ax1.twinx()
    lns2 = ax2.plot(np.transpose(data)[column1][0:],np.transpose(data)[column3][0:], 'r-')
